...
class ridePostViewController: UITableViewController {
    ...
    @IBAction func savePostDetail(sender:UIButton) {
        ...
        self.ref.child("ridePost").child(userId).child(key).child("time").setValue(time)
        self.ref.child("ridePost").child(userId).child(key).child("spotAvail").setValue(spots)
        self.ref.child("ridePost").child(userId).child(key).child("location").setValue(location)
        CurrentUser.sharedInstance.time = time
